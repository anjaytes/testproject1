const { default: harukaConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
function title() {
    console.log(chalk.bold.green(figlet.textSync('SamXCODE - BoT', {
        font: 'Standard',
    ...
    console.log(chalk.yellow(`\n${chalk.yellow('Created By SamXCODE')}\n`))
}

    const haruka = harukaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['SamXCODE Multi Device','Safari','1.0.0'],
        auth: state
    })
	title() 
    ...
    haruka.ev.on('connection.update', async (update) => {
        ...
        console.log('Connect, Welcome SamXCODE')
    })
